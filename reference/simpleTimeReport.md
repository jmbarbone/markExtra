...
This function can be used to evaluate an expression line-by-line to
capture outputs, errors, messages, and evaluation time.

## Usage

``` r
simpleTimeReport(title = NULL, expr, envir = parent.frame(), output = stdout())
```
...
- output:
  ...
  The output connection to write to (default to \`stdout()\`)
