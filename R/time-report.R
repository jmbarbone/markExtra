...
#' @param output The output connection to write to (default to `stdout()`)
...
simpleTimeReport <- function(
  title = NULL,
  expr,
  envir = parent.frame(),
  output = stdout()
) {
  cat0 <- function(...) cat(..., sep = "", file = output)
  ...
  if (!identical(warnings, list()) && any(!vapply(warnings, is.null, NA))) {
    cat("\nWarnings\n", line, "\n", file = output)
  ...
  if (!identical(messages, list()) && any(!vapply(messages, is.null, NA))) {
    cat0("\nMessages\n", line, "\n")
